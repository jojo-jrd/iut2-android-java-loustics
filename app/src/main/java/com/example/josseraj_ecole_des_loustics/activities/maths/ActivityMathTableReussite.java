...
public class ActivityMathTableReussite extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Recupération des données pour l'enregistrement de l'exercice
        this.theme = getIntent().getStringExtra("theme");
        this.sous_theme = getIntent().getStringExtra("sous_theme");
        this.note = getIntent().getStringExtra("note");

        setContentView(R.layout.activity_math_table_reussite);

        // Affichage de la note avec la couleur correspondante
        float ratio = Utilitaire.eval(note);
        TextView noteAffichage = findViewById(R.id.noteMathReussite);
        noteAffichage.setText(this.note);
        if(ratio>0.69){
    ...
    public void menuMath(View view) { enregistrementExercice(false); }

    // Affichage du menu principal
    public void menuPrincipal(View view) { enregistrementExercice(true); }
    ...
    private void enregistrementExercice(boolean menuPrincipal) {
        ...
        class EnregistrementExercice extends AsyncTask<Void, Void, Exercice> {
            ...
            protected Exercice doInBackground(Void... voids) {
                ...
                e.setPseudo(((MyApplication) ActivityMathTableReussite.this.getApplication()).getCompteCourant().getPseudo());

                SimpleDateFormat s = new SimpleDateFormat("dd/MM/yyyy HH:mm");
                Date date = new Date();
                e.setDate(s.format(date));

                e.setTheme(ActivityMathTableReussite.this.theme);
                e.setSous_theme(ActivityMathTableReussite.this.sous_theme);
                e.setResultat(ActivityMathTableReussite.this.note);
            ...
            protected void onPostExecute(Exercice e) {
                ...
                if(menuPrincipal){
                    intent = new Intent(ActivityMathTableReussite.this, ActivityMenuPrincipal.class);
                }else{
                    intent = new Intent(ActivityMathTableReussite.this, ActivityMenuMaths.class);
                }
